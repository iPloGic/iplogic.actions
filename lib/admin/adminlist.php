...
class AdminList extends Page
{
				...
				foreach( $this->arOpts as $arProps ) {
					if( $arProps["FILTER"]["VIEW"] == "date-from-to" || $arProps["FILTER"]["VIEW"] == "num-from-to" ) {
						if( $GLOBALS["find_" . $arProps["NAME"] . "_from"] != "" ) {
							$this->arFilter[">=" . strtoupper($arProps["NAME"])] =
								$GLOBALS["find_" . $arProps["NAME"] . "_from"];
						}
						if( $GLOBALS["find_" . $arProps["NAME"] . "_to"] != "" ) {
							$this->arFilter["<=" . strtoupper($arProps["NAME"])] =
								$GLOBALS["find_" . $arProps["NAME"] . "_to"];
						}
					}
					else {
						if( $GLOBALS["find_" . $arProps["NAME"]] != "" ) {
							$this->arFilter[$arProps["FILTER"]["COMPARE"] . strtoupper($arProps["NAME"])] =
								$GLOBALS["find_" . $arProps["NAME"]];
						}
					}
				}
	...
	public function prepareData($rsData)
	{
						...
						if( isset($options["PARAM"]) ) {
							if( !is_array($options["PARAM"]) ) {
								...
								foreach( $this->arOpts as $arVars ) {
									...
									$param =
										str_replace("##" . $arVars["NAME"] . "_real##", $GLOBALS[$varname_r], $param);
								}
		...
		$arFooter = [];
		if( $this->footerTitle == "Y" ) {
			$arFooter[] = ["title" => $this->Mess["SELECTED"], "value" => $rsData->SelectedRowsCount()];
		}
		if( $this->footerCounter == "Y" ) {
			$arFooter[] = ["counter" => true, "title" => $this->Mess["CHECKED"], "value" => "0"];
		}
		if( count($arFooter) ) {
		...
		if( $this->gaCopy == "Y" ) {
			$this->arGroupActions["copy"] = $this->Mess["COPY"];
		}
		if( $this->gaDelete == "Y" ) {
			$this->arGroupActions["delete"] = $this->Mess["DELETE"];
		}
		if( $this->gaActivate == "Y" ) {
			$this->arGroupActions["activate"] = $this->Mess["ACTIVATE"];
		}
		if( $this->gaDeactivate == "Y" ) {
			$this->arGroupActions["deactivate"] = $this->Mess["DEACTIVATE"];
		}
				...
				switch( $arProps["FILTER"]["VIEW"] ) {
					case 'select':
						echo SelectBoxFromArray(
							$varname,
							$arProps["FILTER"]["VALUES"],
							$GLOBALS[$varname],
							$arProps["FILTER"]["DEFAULT"],
							""
						);
						break;
					case 'date-from-to':
						...
						echo '<div class="adm-filter-alignment">
							<div class="adm-filter-box-sizing">
								<div class="adm-input-wrap" style="display: inline-block;">
									<input type="text" class="adm-input" id="' . $varname_from . '" name="' .
							$varname_from . '" size="15" value="' .
							($GLOBALS[$varname_from] ? $GLOBALS[$varname_from] : '') . '">
								</div>
								<span class="adm-calendar-separate" style="display: inline-block"></span>
								<div class="adm-input-wrap" style="display: inline-block;">
									<input type="text" class="adm-input" id="' . $varname_to . '" name="' .
							$varname_to . '" size="15" value="' . ($GLOBALS[$varname_to] ? $GLOBALS[$varname_to] : '') . '">
								</div>
							</div>
						</div>';
						break;
					default:
						echo '<input type="text" name="find_' . $arProps["NAME"] . '" size="47" value="' .
							htmlspecialchars($GLOBALS[$varname]) . '">';
						break;
				}
				echo "</td></tr>";
			}
		}
		$this->obFilter->Buttons(
			["table_id" => $this->sTableID, "url" => $this->filterFormAction, "form" => "find_form"]
		);
		$this->obFilter->End();
		echo '</form>';
	}
