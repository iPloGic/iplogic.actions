...
$ID = $request->get("ID");
if( $ID > 0 ) {
	$arFields = ActionsTable::getRowById($ID, true);
	if( !$arFields ) {
...
$groupRightsBind = "action";
$module_id = $moduleID;
if( $POST_REQUEST && $MODULE_ACCESS >= "W" ) {
...
$arOpts = [
	...
	"GROUP"                => [
		...
		"TYPE"     => "select",
		"DEFAULT"  => "",
		"NAME"     => Loc::getMessage("IPL_ADMIN_GROUP"),
		"OPTIONS"  => $arGroups,
		"REQURIED" => "Y",
	],
	"NAME"                 => [
	...
	"ACTION"               => [
		...
		"TYPE"     => "text",
		"DEFAULT"  => "",
		"NAME"     => Loc::getMessage("IPL_ADMIN_ACTION"),
		"REQURIED" => "Y",
	],
	"SORT"                 => [
		"TAB"     => "edit1",
	...
	"TYPE"                 => [
		...
		"TYPE"     => "select",
		"DEFAULT"  => "",
		"NAME"     => Loc::getMessage("IPL_ADMIN_TYPE"),
		"OPTIONS"  => [
	...
	"NEW_WINDOW"           => [
		...
		"TYPE"    => "checkbox",
		"DEFAULT" => 'Y',
		"NAME"    => Loc::getMessage("IPL_ADMIN_NEW_WINDOW"),
	],
	"BACKGROUND"           => [
		"TAB"     => "edit1",
		"TYPE"    => "checkbox",
		"DEFAULT" => 'N',
		"NAME"    => Loc::getMessage("IPL_ADMIN_BACKGROUND"),
	],
...
$aMenu = [
	[
		...
		"LINK"  => "iplogic_actions_list.php?lang=" . LANG,
		"ICON"  => "btn_list",
	],
];
...
if( $POST_REQUEST ) {
	...
	if( !count($adminControl->errors) ) {
		...
		if( $ID > 0 ) {
			$res = ActionsTable::update($ID, $arFields);
			if( !$res->isSuccess() ) {
		...
		if( $res ) {
			if( $request->get("apply") != "" ) {
				LocalRedirect(
					"/bitrix/admin/iplogic_actions_edit.php?ID=" . $ID . "&mess=ok&lang=" . LANG . "&" .
					$adminControl->ActiveTabParam()
				);
			}
			else {
				LocalRedirect("/bitrix/admin/iplogic_actions_list.php?lang=" . LANG);
			}
		}
	}
}
