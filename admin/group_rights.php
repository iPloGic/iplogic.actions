...
$ACCESS = new \Iplogic\Actions\Access($groupRightsBind);
...
if( $groupRightsAction == "POST" ) {
	if( strlen($RestoreDefaults) > 0 ) {
		...
		foreach( $ACCESS->arGROUPS as $group ) {
			$tid = ${"TASKS_" . $group["ID"]};
			$arTasksInModule[$group["ID"]] = ['ID' => $tid];
	...
	foreach( $ACCESS->arGROUPS as $group ):
		...
		?>
		<tr valign="top">
			<td><?=$group["NAME"] . " [<a title=\"" . Loc::getMessage("MAIN_USER_GROUP_TITLE") .
				"\" href=\"/bitrix/admin/group_edit.php?ID=" . $group["ID"] . "&amp;lang=" . LANGUAGE_ID . "\">" .
				$group["ID"] . "</a>]:"?></td>
			<td>
				<?
		...
		<tr>
			<td colspan="3">
				...
				<a href="javascript:void(0)" onclick="settingsAddRights(this)" hidefocus="true"
				   class="bx-action-href"><? echo Loc::getMessage("group_rights_add") ?></a>
			</td>
		</tr>
	<? endif ?>
<? } ?>